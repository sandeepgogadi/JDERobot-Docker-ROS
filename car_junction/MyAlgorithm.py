...
class MyAlgorithm(threading.Thread):
    ...
    def setPrevFrames(self, imageL_gray, imageR_gray):
        ...
        self.framePrevL, self.numFramesL = self.saveFrame(
            imageL_gray, self.framePrevL, self.numFramesL)
        self.framePrevR, self.numFramesR = self.saveFrame(
            imageR_gray, self.framePrevR, self.numFramesR)
    ...
    def controlDeviation(self, dev):
        if abs(dev) < self.MAX_DEV:
            ...
            # Turn
            if dev < 0:
                print 'dev: turning to the left, 3.5'
                self.motors.sendW(1.8)
            elif dev > 30:
                self.motors.sendW(-4.0)
            else:
                print 'dev: turning to the right, -3.5'
                self.motors.sendW(-1.8)
    ...
    def execute(self):
        ...
        img2, contours, hierarchy = cv2.findContours(
            image_filtered, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        if (len(contours) != 0):
            ...
            if img_bounding != []:
                ...
                for pt in zip(*loc[::-1]):
                    cv2.rectangle(input_image, (pt[0]+x, pt[1]+y),
                                  (pt[0] + bw+x, pt[1] + bh+y), (0, 0, 255), 2)
                    self.detection = True
                    print("Found signal")
        ...
        if self.stop == True and self.turning == False:
            # Getting the images
            imageL = self.cameraL.getImage().data
            imageR = self.cameraR.getImage().data
        ...
        if self.detectionCar <= self.THRESHOLD_DET:
            ...
            # Turn 45 degrees
            yaw = abs(self.pose3d.getPose3d().yaw * 180/pi)
            ...
            if self.turn45:
                ...
                # Getting the images
                imageC = self.cameraC.getImage().data
                ...
                if middle_lane != 0:
                    cv2.rectangle(imageC, (middle_lane, self.ROW),
                                  (middle_lane + 1, self.ROW + 1), (0, 255, 0), 2)

                    # Calculating the deviation
                    deviation = middle_lane - (columns/2)
                    print('DEVIATION', deviation)

                    # Speed
                    self.controlDeviation(deviation)
